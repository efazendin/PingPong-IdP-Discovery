...
public class PingPongIdpDiscoveryServlet extends HttpServlet {
	...
	private Prioritizer idpPrioritizer;
	...
	public void init() throws ServletException {
		...
		try {
			Class prioritizerClass = Class.forName(this.getServletContext().getInitParameter(IDP_PRIORITIZER));
			idpPrioritizer = (Prioritizer)prioritizerClass.newInstance();
			idpPrioritizer.init(this.getServletContext());
			...
			_logger.error("Prioritizer instantiation failed.", e);
			_logger.warn("Setting prioritizer to: " + NamePrioritizer.class.getCanonicalName());
			idpPrioritizer = new NamePrioritizer();
		}
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Set<IdentityProvider> idpSet = requestorsIdpMap.getValueSet();
		if (appId != null) {
			idpAppFilter = new IdentityProviderAppFilter(appId);
			idpSet = idpAppFilter.filter(idpSet);
		}


		// Prioritize IdPs based on the prioritizer specified in web.xml
		List<IdentityProvider> prioritizedIdps = idpPrioritizer.prioritize(idpSet, req, resp);
